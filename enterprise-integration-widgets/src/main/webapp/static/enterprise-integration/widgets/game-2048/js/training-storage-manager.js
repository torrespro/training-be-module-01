...
TrainingStorageManager.prototype.setBestScore = function (score) {
    if (score && score > 0) {
        ...
        var url = be.contextRoot + "/services/rest/player/" + this.username + "/highscore";
        console.log("PUT", url);
        $.ajax({
            url: url ,
            dataType: "json",
...
TrainingStorageManager.prototype.setGameState = function (gameState) {
    ...
    var url,method;
    if (this.sessionId) {
        url = be.contextRoot + '/services/rest/game/session/' + this.sessionId;
        method = "PUT";
    } else {
        url = be.contextRoot + '/services/rest/game/session';
        method = "POST";
    }
